...
pub async fn create_application(
...
) -> Result<(axum::http::StatusCode, Json<Application>), ApiError> {
    pliq_back_db::queries::listings::get_by_id(&state.db, listing_id)
        .await?
    ...
    let has_active = pliq_back_db::queries::applications::has_active_application(
        &state.db, auth.user_id, listing_id,
    )
    .await
    .map_err(|e| ApiError::Internal(e.to_string()))?;
    if has_active {
        return Err(ApiError::Conflict("Active application already exists".into()));
    }
    ...
    let new = NewApplication {
        ...
        zk_proof_hash: body.zk_proof_hash,
        credential_summary: body.credential_summary,
    };
...
pub async fn list_applications(
    ...
    Ok(Json(
        pliq_back_db::queries::applications::list_by_tenant(
            &state.db, auth.user_id, &pagination,
        )
        .await?,
    ))
}
...
pub async fn get_application(
    ...
    let app = pliq_back_db::queries::applications::get_by_id(&state.db, id)
        .await?
...
pub async fn update_application_status(
    ...
    let status_str = body
        .get("status")
        .and_then(|v| v.as_str())
        .ok_or_else(|| ApiError::Validation("status required".into()))?;
    let status = match status_str {
        "accepted" => ApplicationStatus::Accepted,
        "rejected" => ApplicationStatus::Rejected,
        "under_review" => ApplicationStatus::UnderReview,
        _ => return Err(ApiError::Validation("Invalid status".into())),
    };
    Ok(Json(
        pliq_back_db::queries::applications::update_status(&state.db, id, status).await?,
    ))
}
...
pub async fn withdraw_application(
    ...
    let app = pliq_back_db::queries::applications::get_by_id(&state.db, id)
        .await?
        .ok_or_else(|| ApiError::NotFound("Application not found".into()))?;
    if app.tenant_id != auth.user_id {
        return Err(ApiError::Forbidden("Not the applicant".into()));
    }
    Ok(Json(
        pliq_back_db::queries::applications::update_status(
            &state.db, id, ApplicationStatus::Withdrawn,
        )
        .await?,
    ))
}
